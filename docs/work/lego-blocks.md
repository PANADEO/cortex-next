...
```ts
type ProcessingState = "imported" | "imported_with_error" | "analysing" | "analysis_failed" | "ready";
...
interface UploadItem { id: string; file: File; progress: number; status: FileStatus; error?: string }
...
interface DataCardProps {
  ...
  icon?: LucideIcon; isLoading?: boolean; onClick?: () => void;
}
```
...
```ts
interface ThemeToggleProps { mode?: "icon" | "labeled" }
```
...
```ts
interface CopyButtonProps { value: string; successMessage?: string; size?: "icon" | "sm" }
```
...
```ts
interface DateDisplayProps { value: string | Date; format?: "relative" | "absolute" | "both"; tz?: string }
```
...
```ts
interface AuthLayoutProps { brand?: ReactNode; children: ReactNode; footer?: ReactNode }
```
...
interface TileMenuItem {
  ...
  badge?: string | number; disabled?: boolean;
}
interface TileMenuSection { id: string; label?: string; items: TileMenuItem[] }
