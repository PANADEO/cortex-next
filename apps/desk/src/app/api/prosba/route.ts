...
export async function GET() {
  ...
  const r = u.rola === 'zarzad'
    ? await pool.query(`select * from desk.prosba order by (stan='oczekuje') desc, at desc limit 50`)
...
export async function POST(req: Request) {
  ...
  const istnieje = await pool.query(
    `select id from desk.prosba where kto=$1 and zdolnosc=$2 and stan='oczekuje'`, [u.id, zdolnosc],
  )
...
export async function PATCH(req: Request) {
  ...
  if (cofnij) {
    ...
    await pool.query(`update desk.prosba set stan='odrzucona', rozpatrzona=now(), rozpatrzyl=$2 where id=$1`, [id, u.id])
