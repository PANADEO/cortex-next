...
export function Podglad({ plik }: { plik: PlikMeta }) {
  ...
  if (ext === 'pdf')
    return <iframe src={url(plik)} title={plik.nazwa} className="h-[70vh] w-full rounded-md border" />
      ...
      <div>
        <div className="overflow-x-auto rounded-md border">
          <table className="w-full border-collapse text-[13px] tabular-nums">
            ...
            <tbody>
              {reszta.map((w, i) => (
                <tr key={i}>{w.split(sep).map((c, j) => (
                  <td key={j} className="border-b px-2.5 py-1.5">{c}</td>
                ))}</tr>
              ))}
            </tbody>
          </table>
        ...
        {wiersze.length > MAX_WIERSZY + 1 && (
          <p className="pt-1.5 t-micro">pokazuję {MAX_WIERSZY} z {wiersze.length - 1} wierszy</p>
        )}
      </div>
    )
  }

  return <pre className="overflow-x-auto whitespace-pre-wrap rounded-md bg-sunken p-3 font-mono text-[13px] leading-5">{tresc}</pre>
}
