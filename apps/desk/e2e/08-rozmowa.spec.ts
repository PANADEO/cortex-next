...
test.describe('Obszar 8 · Rozmowa, którą da się prowadzić', () => {
  test('Zaznaczenie tekstu przeżywa odświeżanie w tle', async ({ page, request }) => {
    test.setTimeout(120_000)
    await jako(page, 'anna')
    const r = await request.post('/api/sprawa/nowa', {
      headers: { Cookie: 'desk_persona=anna' }, data: { tytul: 'Zaznaczanie' },
    })
    ...
    expect((await page.evaluate(() => window.getSelection()?.toString() ?? '')).length).toBeGreaterThan(10)
  ...
  test('Polecenie stoi po prawej, odpowiedź po lewej', async ({ page, request }) => {
    test.setTimeout(120_000)
    await jako(page, 'anna')
    const r = await request.post('/api/sprawa/nowa', {
      headers: { Cookie: 'desk_persona=anna' }, data: { tytul: 'Strony' },
    })
    const { id } = await r.json()
    await request.post(`/api/sprawa/${id}/tura`, {
      headers: { Cookie: 'desk_persona=anna' },
  ...
  test('Agent odpowiada na zwykłe pytanie, zamiast odsyłać do plików', async ({ page, request }) => {
    test.setTimeout(120_000)
    await jako(page, 'anna')
    const r = await request.post('/api/sprawa/nowa', {
      headers: { Cookie: 'desk_persona=anna' }, data: { tytul: 'Pytanie ogólne' },
    })
    const { id } = await r.json()
    await request.post(`/api/sprawa/${id}/tura`, {
      headers: { Cookie: 'desk_persona=anna' },
  ...
  test('Panel wyniku da się schować i zostaje schowany', async ({ page, request }) => {
    await jako(page, 'anna')
    const r = await request.post('/api/sprawa/nowa', {
      headers: { Cookie: 'desk_persona=anna' }, data: { tytul: 'Panel' },
    })
    const { id } = await r.json()
    await page.goto(`/sprawa/${id}`)
  ...
  test('Wskaźnik pracy pojawia się od razu po wysłaniu, nie po pierwszym kroku', async ({ page, request }) => {
    test.setTimeout(120_000)
    await jako(page, 'anna')
    const r = await request.post('/api/sprawa/nowa', {
      headers: { Cookie: 'desk_persona=anna' }, data: { tytul: 'Wskaźnik' },
    })
    const { id } = await r.json()
    await page.goto(`/sprawa/${id}`)
...
test.describe('Obszar 9 · „Moje pliki" to przestrzeń świadomych decyzji', () => {
  test('Załącznik do rozmowy trafia do teczki sprawy, nie do Moich plików', async ({ page, request }) => {
    test.setTimeout(120_000)
    await jako(page, 'anna')
    const r = await request.post('/api/sprawa/nowa', {
      headers: { Cookie: 'desk_persona=anna' }, data: { tytul: 'Załącznik' },
    })
    const { id } = await r.json()

    await page.goto(`/sprawa/${id}`)
    await page.locator('input[type=file]').first().setInputFiles({
      name: 'zalacznik-testowy.txt', mimeType: 'text/plain', buffer: Buffer.from('zawartość załącznika'),
    })
    ...
    const pliki = await (await request.get('/api/pliki', { headers: { Cookie: 'desk_persona=anna' } })).json()
    const wMoichPlikach = (pliki.pliki ?? []).some((x: { nazwa: string }) => x.nazwa === 'zalacznik-testowy.txt')
    expect(wMoichPlikach).toBe(false)

    const teczka = await (await request.get(`/api/sprawa/${id}/zdarzenia?od=0`, { headers: { Cookie: 'desk_persona=anna' } })).json()
    expect((teczka.teczka ?? []).some((x: { nazwa: string }) => x.nazwa === 'zalacznik-testowy.txt')).toBe(true)
  })

  test('Agent odkłada plik do Moich plików dopiero na wyraźną prośbę', async ({ request }) => {
    ...
    await request.post(`/api/sprawa/${id}/tura`, {
      headers,
      data: { tresc: 'Zapisz plik notatka-testowa.md z jednym zdaniem o kosztach, a potem odłóż go do Moich plików.' },
    })
    ...
    expect((pliki.pliki ?? []).some((x: { nazwa: string }) => x.nazwa.startsWith('notatka-testowa'))).toBe(true)
