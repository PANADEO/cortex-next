...
test.describe("Obszar 3 · Zlecam robotę, dostaję dokument z dowodem", () => {
  ...
  test("Praca na pliku z biurka kończy się dokumentem z dowodem", { tag: "@model" }, async ({ page }) => {
  ...
  test('Zakończony przebieg mówi w czasie przeszłym, nie „zapisuję" o czymś zapisanym', { tag: "@model" }, async ({
    page,
  }) => {
    test.setTimeout(180_000)
    await jako(page, "anna")
    ...
    const d = dowodZeZdarzen([
      {
        ...
        id: "a",
        nazwa: "czytaj_plik",
        etykieta: "Czytam a.csv",
        argumenty: { sciezka: "a.csv" },
      },
      {
      ...
      },
      {
        ...
        etykieta: "Zapisuję w.md",
        argumenty: { nazwa: "w.md" },
      },
      {
        typ: "narzedzie_koniec",
        id: "b",
...
test.describe("Plakietka sprawdzenia mówi tylko to, co widać w zdarzeniach", () => {
  test('Obraz nie dostaje plakietki „sprawdzony" — nikt go po zapisie nie odczytał', { tag: "@model" }, async ({
    page,
    request,
  }) => {
    test.setTimeout(180_000)
