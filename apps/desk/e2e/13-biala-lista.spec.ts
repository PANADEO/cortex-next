...
async function nadaj(request: APIRequestContext, zdolnosc: string) {
  ...
  const r = await request.patch("/api/prosba", {
    headers: ROBERT,
...
async function tura(request: APIRequestContext, tytul: string, tresc: string) {
  const { id } = await (
    await request.post("/api/sprawa/nowa", { headers: ANNA, data: { tytul } })
  ).json()
  ...
  for (let i = 0; i < 80; i++) {
    ...
    if (d.sprawa.stan !== "pracuje" && d.sprawa.stan !== "nowa")
      return { id, zdarzenia: d.zdarzenia as { event: DeskEvent }[] }
  }
...
const nazwy = (z: { event: DeskEvent }[]) =>
  z
    .filter((x) => x.event.typ === "narzedzie_start")
    .map((x) => (x.event as { nazwa: string }).nazwa)
...
test.describe("Obszar 24 · Zdolność sięgająca poza firmę przechodzi tą samą bramą", () => {
  test("Wykaz jest zatwierdzony narzędzie po narzędziu, nie w całości", () => {
    ...
    expect(new Set(NARZEDZIA_BIALEJ_LISTY.map((n) => n.odcisk)).size).toBe(
      NARZEDZIA_BIALEJ_LISTY.length,
    )
  })
  ...
  test("Bez zgody przełożonego agent nie dostaje narzędzia z wykazu, tylko zgłasza brak", { tag: "@model" }, async ({
    request,
  }) => {
    test.setTimeout(200_000)
    await request.post("/api/test/reset-uprawnien")
    const { zdarzenia } = await tura(
      request,
  ...
  test("Po nadaniu agent naprawdę odpytuje wykaz Ministerstwa Finansów", { tag: "@model" }, async ({ request }) => {
    test.setTimeout(200_000)
    await request.post("/api/test/reset-uprawnien")
    ...
    const odpowiedz = zdarzenia
      .filter((x) => x.event.typ === "assistant")
      .map((x) => (x.event as { tekst: string }).tekst)
      .join(" ")
...
test.describe("Obszar 25 · Odpowiedź obcego serwera nie udaje wykonanej pracy", () => {
  const obce: DeskEvent[] = [
    {
      typ: "narzedzie_start",
      id: "z",
      nazwa: "mcp_biala_lista_sprawdz_rachunek",
      etykieta: "sprawdzenie rachunku w wykazie",
      zrodlo: "wykaz podatników VAT",
    ...
    },
    {
      ...
      nazwa: "mcp_biala_lista_sprawdz_rachunek",
      ok: true,
      podsumowanie: "serwer odpowiedział",
      ms: 110,
    },
  ]
  ...
  test("Przebieg mówi o odpytaniu wykazu, nie o slugu serwera", { tag: "@model" }, async ({ page, request }) => {
    test.setTimeout(200_000)
    await request.post("/api/test/reset-uprawnien")
    await nadaj(request, "kontrahent.sprawdz")
    const { id } = await tura(
