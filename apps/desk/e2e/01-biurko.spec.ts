...
async function odslonPodpowiedzi(page: Page) {
  await page.waitForSelector('button:has-text("Podpowiedzi"), button:has-text("Zestawienie kosztów")')
...
test.describe('Obszar 1 · To jest MOJE biurko', () => {
  ...
  test('Podpowiedzi są zawsze osiągalne — na pustym biurku wprost, później po rozwinięciu', async ({ page }) => {
    await jako(page, 'anna')
    await page.goto('/')
    await odslonPodpowiedzi(page)
    const karty = page.locator('button', { hasText: /Zestawienie kosztów|Notatka ze spotkania|brakuje w dokumencie/ })
  ...
  test('Kliknięcie podpowiedzi wstawia treść do pola, ale nie wysyła', async ({ page }) => {
    ...
    await page.goto('/')
    await odslonPodpowiedzi(page)
    await page.locator('button', { hasText: /Zestawienie kosztów/ }).first().click()
