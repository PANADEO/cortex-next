...
test.describe("Obszar 4 · Praca nie ginie", () => {
  test("Odświeżenie w trakcie tury pokazuje znacznik pracy i cały przebieg", { tag: "@model" }, async ({
    page,
    request,
  }) => {
    ...
    const h = await request.get(`/api/sprawa/${id}/zdarzenia?od=0`, {
      headers: { Cookie: "desk_persona=anna" },
    })
    const d = await h.json()
    expect(d.zdarzenia.some((z: Zdarzenie) => z.event.typ === "mysl")).toBe(true)
    expect(
      d.zdarzenia.filter((z: Zdarzenie) => z.event.typ === "narzedzie_start").length,
    ).toBeGreaterThan(0)
  })

  test("Stop kończy turę jako przerwaną, nie jako błąd", { tag: "@model" }, async ({ page, request }) => {
