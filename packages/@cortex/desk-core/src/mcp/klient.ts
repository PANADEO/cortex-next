...
export async function narzedziaMcp(
  ...
  for (const serwer of await katalogSerwerow()) {
    ...
    try {
      ...
      for (const n of dozwolone) {
        ...
        if (!surowe)
          throw new DryfNarzedzia(`Serwer ${serwer.nazwa} nie wystawia już ${n.nazwaZdalna}.`)
        ...
        const surowyJson =
          (surowe.inputSchema as { jsonSchema?: unknown })?.jsonSchema ?? surowe.inputSchema
        ...
        if (teraz !== n.odcisk) {
          ...
          await zdarz({
            typ: "zablokowane",
            opis: `${n.krotko} — ${powod} Czynność czeka na ponowną zgodę przełożonego.`,
          })
          continue
        }
        ...
        dopiszKarte({
          ...
          klasa: "zewnetrzna",
          zrodlo: serwer.nazwa,
          trwa: `Pytam ${serwer.etykieta}`,
          ok: `Zapytałem ${serwer.etykieta}`,
          grupa: {
            klucz: `zewnetrzne:${serwer.nazwa}`,
            czasownik: `zapytałem ${serwer.etykieta}`,
            liczone: ["raz", "razy", "razy"],
            waga: 4,
          },
          dowod: {
            lista: "weszlo",
            fraza: (_x, d) => `${serwer.etykieta}: ${n.opis}${d ? ` — ${d}` : ""}`,
          },
        })
        ...
        wynik[klucz] = tool({
          ...
          execute: async (argumenty: unknown) => {
            ...
            await zdarz({
              ...
              nazwa: klucz,
              etykieta: n.krotko,
              zrodlo: serwer.etykieta,
              argumenty: argumenty as Record<string, unknown>,
            })
            try {
              const r = await (surowe as { execute: (a: unknown) => Promise<unknown> }).execute(
                argumenty,
              )
      ...
      await zdarz({
        typ: "zablokowane",
        opis:
          e instanceof DryfNarzedzia || e instanceof SchematOdrzucony
            ? e.message
  ...
  return {
    narzedzia: wynik,
    zamknij: async () => {
      for (const k of otwarte) await k.close().catch(() => {})
    },
  }
}
...
export async function przejrzyjSerwer(url: string, nazwa: string): Promise<KandydatNarzedzia[]> {
  ...
  try {
    ...
    return Object.entries(
      zdalne as Record<string, { inputSchema?: unknown; description?: string }>,
    ).map(([nazwaZdalna, def]) => {
      const surowyJson =
        (def.inputSchema as { jsonSchema?: unknown })?.jsonSchema ?? def.inputSchema
      try {
        const schemat = oczyscSchemat(surowyJson)
        return {
          nazwaZdalna,
        ...
        return {
          ...
          obcyOpis: def.description ?? null,
          odrzucone:
            e instanceof SchematOdrzucony
              ? e.message
              : "Nie umiem odczytać schematu tego narzędzia.",
        }
      }
    })
  } finally {
