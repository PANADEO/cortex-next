...
export function opiszWpis(w: Wpis): { tekst: string; waga: "zwykly" | "wazny" } {
  ...
  switch (w.typ) {
    ...
    case "tura.start":
      return {
        tekst: `zleciła pracę · zakres uprawnień ${s.odcisk ?? "?"} (${Array.isArray(s.zdolnosci) ? s.zdolnosci.length : "?"} zdolności)`,
        waga: "zwykly",
      }
    case "tura.koniec":
    ...
    case "prosba.przyznana":
      return {
        tekst: `przyznał zdolność „${nazwaZdolnosci(s.zdolnosc)}” osobie ${imie(s.komu)}`,
        waga: "wazny",
      }
    case "prosba.odrzucona":
      return {
        tekst: `odmówił zdolności „${nazwaZdolnosci(s.zdolnosc)}” osobie ${imie(s.komu)}`,
        waga: "wazny",
      }
    case "prosba.inne":
    ...
    case "zdolnosc.cofnieta":
      return {
        tekst: `cofnął zdolność „${nazwaZdolnosci(s.zdolnosc)}” osobie ${imie(s.komu)}`,
        waga: "wazny",
      }
    case "zdolnosc.brak":
    ...
    case "mcp.narzedzie.zatwierdzone":
      return {
        tekst: `przyjęła narzędzie ${s.narzedzie ?? ""} z serwera ${s.serwer ?? ""}`,
        waga: "wazny",
      }
    case "mcp.narzedzie.wycofane":
      return {
        tekst: `wycofała narzędzie ${s.narzedzie ?? ""} z serwera ${s.serwer ?? ""}`,
        waga: "wazny",
      }
    case "mcp.narzedzie.wstrzymane":
      // to jedyny wpis, który zapisuje sama aplikacja bez udziału człowieka
      return {
        tekst: `— narzędzie ${s.narzedzie ?? ""} wstrzymane: ${s.powod ?? ""}`,
        waga: "wazny",
      }
    case "koszt.wyzerowany":
      return {
        tekst: `— wyzerowano dzisiejszy koszt (${s.usd ?? 0} USD na ${s.spraw ?? 0} sprawach)`,
        waga: "zwykly",
      }
    default:
