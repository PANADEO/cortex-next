...
export function Klodka({
  ...
  async function popros() {
    ...
    const r = await fetch(api("/prosba"), {
      method: "POST",
      body: JSON.stringify({ zdolnosc: zdolnoscId }),
    })
    setZajete(false)
    if (!r.ok) {
    ...
    <div className="flex max-w-miara gap-2.5 rounded-lg border bg-surface px-3.5 py-3">
      ...
      <div className="min-w-0">
        <div className="t-tresc">
          {nazwa ? (
            <>
              Do tego potrzebuję zdolności <span className="font-medium">„{nazwa}”</span>, której
              nie masz włączonej.
            </>
          ) : (
        ...
        <div className="t-meta mt-0.5">
          Chodziło o: {opis}
          {dzial ? ` · zgodę wydaje dział ${dzial}` : ""}
        </div>
        {zdolnoscId &&
          (wyslane ? (
