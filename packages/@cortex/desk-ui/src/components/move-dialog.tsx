...
export function MoveDialog({
  ...
  async function create() {
    ...
    if (!n) return
    const path = `${selected ?? MY_FILES}/${n}`
                ...
                <li key={k}>
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1 truncate">
                      {k === MY_FILES ? translate("files.myFilesFolder") : k.split("/").pop()}
                    </span>
                    {here && (
                      <span className="t-micro shrink-0">{translate("move.alreadyHere")}</span>
                    )}
                    {isSelected && (
