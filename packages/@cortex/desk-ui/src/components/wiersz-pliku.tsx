...
export function WierszPliku({
  ...
  function skroty(e: React.KeyboardEvent) {
    if (edycja) return
    if (e.key === "F2" && akcje.zmienNazwe) {
      e.preventDefault()
      setEdycja(true)
    }
    if (e.key === "Delete" && akcje.usun) {
      e.preventDefault()
      akcje.usun(p)
    }
  }
    ...
    <li className={aktywny ? "bg-raised" : undefined}>
      ...
      >
        ...
        {edycja ? (
          ...
          <button onClick={glowna} className="t-tresc-m flex min-w-0 flex-1 items-center text-left">
            <span className="truncate">{rdzen}</span>
            <span className="shrink-0 text-cichy">{ext}</span>
          </button>
        )}

        <span className="t-meta hidden shrink-0 sm:block">
          {p.katalog ? "" : rozmiar(p.rozmiar)}
        </span>
        ...
        <Menu.Root>
          ...
          <Menu.Portal>
            ...
            >
              {!p.katalog && akcje.podglad && (
                <Pozycja
                  ikona={Eye}
                  etykieta="Otwórz podgląd"
                  skrot="Enter"
                  na={() => akcje.podglad?.(p)}
                />
              )}
              {!p.katalog && akcje.pobierz && (
                <Pozycja ikona={Download} etykieta="Pobierz" na={() => akcje.pobierz?.(p)} />
              )}
              {akcje.zmienNazwe && <Rozdzielacz />}
              {akcje.zmienNazwe && (
                <Pozycja
                  ikona={Pencil}
                  etykieta="Zmień nazwę"
                  skrot="F2"
                  na={() => setEdycja(true)}
                />
              )}
              ...
              {akcje.doMoichPlikow && (
                <Pozycja
                  ikona={FolderOutput}
                  etykieta="Zapisz do Moich plików"
                  na={() => akcje.doMoichPlikow?.(p)}
                />
              )}
...
function Pozycja({
  ...
  return (
    <Menu.Item
      onSelect={na}
      className={`t-tresc flex cursor-pointer items-center gap-2.5 px-3 py-1.5 outline-none data-[highlighted]:bg-raised ${grozny ? "text-bad" : ""}`}
    >
