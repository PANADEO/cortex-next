 ...
 */
export function Artefakty({ pliki, otworz }: { pliki: PlikMeta[]; otworz: (p: PlikMeta) => void }) {
  ...
  return (
    <figure className="max-w-[420px]">
      ...
      <figcaption className="flex items-center gap-2 px-0.5 pt-1.5">
        <span className="t-meta min-w-0 flex-1 truncate">{plik.nazwa}</span>
        <a
          href={adresPliku(plik, true)}
          download
          title="Pobierz"
          aria-label={`Pobierz ${plik.nazwa}`}
          className="grid h-7 w-7 shrink-0 place-items-center rounded-sm text-cichy hover:bg-raised hover:text-ink"
        >
          <Ikona jako={Download} px={14} />
        </a>
      </figcaption>
    </figure>
  )
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="t-tresc-m block truncate">{plik.nazwa}</span>
        <span className="t-meta block">
          Dokument · {rodzaj(plik.nazwa)} · {rozmiar(plik.rozmiar)}
        </span>
      </span>
