...
import { czasKroku, opisKroku, paruj, podsumujGrupe, type Krok } from "@cortex/desk-core/kroki"
  ...
  return (
    <li>
      ...
      >
        ...
        {czas && <span className="t-meta shrink-0 tabular-nums">{czas}</span>}
        {maSzczegol && (
          <Ikona
            jako={ChevronDown}
            px={14}
...
export function Przebieg({
  ...
  useEffect(() => {
    ...
    if (potkniecie || niepewne || zablokowane) {
      zwijano.current = true
      return
    }
    const t = setTimeout(() => {
      setZwiniety(true)
  ...
  const naglowek = pracuje
    ...
    : potkniecie
      ? {
          ikona: TriangleAlert,
          klasa: "text-warn",
          tekst: `Zrobione z potknięciem: ${podsumujGrupe(kroki).toLowerCase()}`,
        }
    ...
    >
      <h3>
        ...
        >
          ...
          <span className="t-tresc-m min-w-0 flex-1 truncate">{naglowek.tekst}</span>
          {pracuje && trwajacy && (
            <span className="t-meta shrink-0">krok {kroki.indexOf(trwajacy) + 1}</span>
          )}
          {!pracuje && czasCalosc && (
            <span className="t-meta shrink-0 tabular-nums">{czasCalosc}</span>
          )}
          {!pracuje && niepewne && (
            <span className="t-meta flex shrink-0 items-center gap-1 text-warn">
              <Ikona jako={TriangleAlert} px={12} />
              {dowod.nieSprawdzone.length === 1
                ? "1 rzecz niesprawdzona"
                : `${dowod.nieSprawdzone.length} rzeczy niesprawdzone`}
            </span>
          )}
          <Ikona
            jako={ChevronDown}
      ...
      {(dowod.weszlo.length > 0 ||
        dowod.zrobione.length > 0 ||
        zewnetrzne ||
        niepewne ||
        zablokowane) && (
