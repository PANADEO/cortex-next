...
export function Zlecenia({
  ...
  async function start(t: string) {
    ...
    try {
      ...
      const r = await fetch(api("/sprawa/nowa"), {
        method: "POST",
      ...
      pole.current?.focus()
      pokaz({
        tekst: e instanceof Error ? e.message : "Nie udało się zacząć sprawy. Spróbuj jeszcze raz.",
        ton: "blad",
      })
    }
  }
    ...
    <div>
      <div className="rounded-xl border bg-surface shadow-pop">
        ...
        <div className="flex items-center gap-1 px-2.5 pb-2.5">
          ...
          <button
            onClick={() => start(tresc)}
            disabled={(!tresc.trim() && !zal.length) || zajete}
          ...
          <div className="grid gap-2.5 sm:grid-cols-2">
            {zlecenia.map((z) => (
              ...
              >
                ...
                <div className="t-meta mt-0.5">{z.podpowiedz}</div>
              </button>
            ))}
          </div>
        </>
