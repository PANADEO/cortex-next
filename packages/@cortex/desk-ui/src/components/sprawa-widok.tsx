...
import { SZER_DOM, UchwytPanelu, zacisnij } from "./uchwyt"
...
export function SprawaWidok({ id, polityka: p }: { id: string; polityka: Polityka }) {
  ...
  useEffect(() => {
    ...
    async function tick() {
      try {
        const r = await fetch(`${api("")}/sprawa/${id}/zdarzenia?od=${od.current}`, {
          cache: "no-store",
        })
        if (!r.ok || !zyje) return
        ...
        setSprawa((s) =>
          s &&
          s.stan === d.sprawa.stan &&
          s.tytul === d.sprawa.tytul &&
          s.koszt === d.sprawa.koszt &&
          s.powod === d.sprawa.powod
            ? s
            : d.sprawa,
        )
        setTeczka((t) => {
          ...
          const takieSame =
            t.length === nowa.length &&
            t.every(
              (x, i) =>
                x.sciezka === nowa[i]?.sciezka &&
                x.rozmiar === nowa[i]?.rozmiar &&
                x.zmieniony === nowa[i]?.zmieniony,
            )
        ...
        if (d.zdarzenia?.length) {
          ...
          const start = d.zdarzenia.find(
            (z: Wpis) =>
              z.event.typ === "lifecycle" && (z.event as { stan?: string }).stan === "start",
          )
  ...
  const poNazwie = useCallback((n: string) => teczka.find((x) => x.nazwa === n) ?? null, [teczka])

  /** Jedno wejście do podglądu — z karty artefaktu, z załącznika w rozmowie i z panelu. */
  const pokazPlik = useCallback(
    (plik: PlikMeta | null) => {
      if (!plik) return
  ...
  const doDowodu = () =>
    stopkaDowodu.current?.scrollIntoView({ behavior: "smooth", block: "center" })
  ...
  const tablica = (
    <Wynik
      ...
      aktywny={aktywny}
      naWybor={(x) => setWybrany(x.sciezka)}
      dowod={dowod}
      doDowodu={doDowodu}
    />
  )
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-pasek shrink-0 items-center gap-2 border-b bg-surface px-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="t-h3 truncate">{sprawa?.tytul ?? "Sprawa"}</div>
            <div className="t-meta flex items-center gap-1.5">
              ...
              {pracuje ? (
                <span className="text-akcent">
                  pracuje · {sekundy < 60 ? `${sekundy} s` : `${Math.round(sekundy / 60)} min`}
                </span>
              ) : (
          ...
          {pracuje && (
            ...
            >
              <Ikona jako={Square} px={14} /> Stop
            </button>
          )}
          <button
            onClick={() => ustawPanel(!panel)}
            aria-pressed={panel}
            ...
            <Menu.Portal>
              ...
              >
                <div className="t-sekcja flex items-center gap-2 pb-2">
                  <Ikona jako={Info} px={14} /> Szczegóły sprawy
                </div>
                <dl className="t-meta space-y-1">
                  <div className="flex justify-between gap-4">
                    <dt>Czynności</dt>
                    <dd className="text-ink">
                      {wpisy.filter((w) => w.event.typ === "narzedzie_start").length}
                    </dd>
                  </div>
                  <div className="flex justify-between gap-4">
                    <dt>Dokumenty</dt>
                    <dd className="text-ink">{wyniki.length}</dd>
                  </div>
                  <div className="flex justify-between gap-4">
                    <dt>Koszt</dt>
                    <dd className="text-ink">{zl(sprawa?.koszt ?? 0)}</dd>
                  </div>
                  <div className="flex justify-between gap-4">
                    <dt>Uprawnienia</dt>
                    <dd className="text-ink">
                      jak w dziale {p.rola === "zarzad" ? "Zarząd" : "Księgowość"} (
        ...
        >
          <div className="mx-auto flex max-w-strumien flex-col gap-4">
            {tury.map((tura, i) => {
              ...
              const artefakty = wytworzone(zdarzeniaTury)
                .map(poNazwie)
                .filter((x): x is PlikMeta => Boolean(x))
              return (
          ...
          {!przyDole && (
            ...
            >
              <Ikona jako={ArrowDown} px={14} /> Nowe kroki
            </button>
          )}
        </div>
        ...
        {aktywny && (
          ...
          >
            ...
            <span className="t-tresc-m min-w-0 flex-1 truncate">{aktywny.nazwa}</span>
        ...
        <div className="shrink-0 border-t bg-surface p-3">
          <div className="edytor mx-auto max-w-strumien rounded-xl border bg-bg">
            ...
            <textarea
              ...
              placeholder={
                zajete ? "Pracuję — poczekaj albo naciśnij Stop" : "Napisz, co mam zrobić…"
              }
            ...
            <div className="flex items-center gap-1 px-2 pb-2">
              ...
              >
                <Ikona
                  jako={zajete ? LoaderCircle : ArrowUp}
                  px={16}
                  klasa={zajete ? "obrot" : undefined}
                />
              </button>
            </div>
          </div>
