...
export function DialogPrzenies({
  plik,
  zamknij,
  przenies,
}: {
  ...
  async function utworz() {
    ...
    await fetch(api("/pliki"), {
      method: "POST",
      body: JSON.stringify({ akcja: "katalog", sciezka }),
    })
              ...
              return (
                <li key={k}>
                  <button
                    ...
                    className={`t-tresc flex h-9 w-full items-center gap-2 rounded-sm pr-2 text-left disabled:opacity-45 ${zaznaczony ? "bg-raised" : "hover:bg-raised/60"}`}
                  >
                    <Ikona jako={Folder} px={16} klasa="shrink-0 text-cichy" />
                    <span className="min-w-0 flex-1 truncate">{k.split("/").pop()}</span>
                    {tutaj && <span className="t-micro shrink-0">plik już tu jest</span>}
              ...
              {tworzy ? (
                <div className="flex items-center gap-2 px-2 py-1">
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") void utworz()
                      if (e.key === "Escape") setTworzy(false)
                    }}
                    placeholder="Nazwa folderu"
                    aria-label="Nazwa nowego folderu"
                    className="t-tresc min-w-0 flex-1 rounded-sm border bg-bg px-1.5 py-1 outline-none"
                  />
                  <button
                    onClick={() => void utworz()}
                    className="t-btn rounded-sm px-2 py-1 text-akcent hover:bg-raised"
                  >
                    Utwórz
                  </button>
                </div>
                ...
                <button
                  onClick={() => setTworzy(true)}
                  className="t-tresc flex h-9 w-full items-center gap-2 rounded-sm px-2 text-left text-cichy hover:bg-raised/60"
                >
