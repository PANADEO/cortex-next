...
export function ListaZdolnosci({
  p,
  gesta,
  szukanie,
}: {
  p: Polityka
  gesta?: boolean | undefined
  szukanie?: boolean
}) {
  ...
  async function popros(id: string, nazwa: string) {
    ...
    setWyslane((w) => [...w, id])
    pokaz({
      tekst: `Prośba o „${nazwa}” poszła do działu. Dostaniesz znać, gdy ktoś ją rozpatrzy.`,
    })
  }

  const pasuje = (z: Zdolnosc) =>
    !fraza.trim() ||
    `${z.nazwa} ${z.opis} ${z.dzial}`.toLowerCase().includes(fraza.trim().toLowerCase())
          ...
          poDzialach(mam).map(([dzial, zd]) => (
            <div key={dzial} className="mb-2.5">
              <div className="t-micro px-1 pb-1">
                {dzial === "wszyscy" ? "Dla wszystkich" : dzial}
              </div>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {wyslane.includes(z.id) ? (
                    ...
                    <>
                      ...
                      >
                        {odrzucone.includes(z.id) ? "Poproś ponownie" : "Poproś o dostęp"}
                      </button>
                    </>
                  )}
                </div>
    ...
    <Menu.Root open={otwarty} onOpenChange={setOtwarty}>
      <Menu.Trigger className="flex items-center gap-1.5 rounded-sm px-2 py-1 text-[13px] text-cichy hover:bg-raised hover:text-ink">
        ...
        <Ikona
          jako={ChevronDown}
          px={14}
          klasa={`transition-transform ${otwarty ? "rotate-180" : ""}`}
        />
      </Menu.Trigger>
      <Menu.Portal>
        <Menu.Content
