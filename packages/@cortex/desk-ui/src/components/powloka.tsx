...
export async function Powloka({
  ...
  const wszystkich = await pool.query<{ n: string }>(
    `select count(*)::text as n from desk.sprawa where wlasciciel=$1`,
    [u.id],
  )
        ...
        <aside className="hidden w-boczny shrink-0 flex-col border-r bg-surface md:flex">
              ...
              {widoczne.map((r) => (
                <li key={r.id}>
                  ...
                  >
                    ...
                    <span
                      className={`h-2 w-2 shrink-0 rounded-pill ${KROPKA[r.stan] ?? "bg-cichy-2"}`}
                    />
                    <span className="min-w-0 flex-1 truncate">{r.tytul}</span>
                    <span className="t-micro shrink-0">{kiedy(r.zmieniona.toISOString())}</span>
                  </Link>
                </li>
              ))}
              {razem > W_PASKU && (
          ...
          <div className="border-t p-3">
            ...
            >
              ...
              Umiem {p.przyznane.length} z{" "}
              {ile(p.przyznane.length + p.zablokowane.length, "rzeczy", "rzeczy", "rzeczy")}
