...
export default async function Strona() {
  ...
  const wydatki = await Promise.all(
    UZYTKOWNICY.map(async (x) => ({
      osoba: x,
      usd: await wydanoDzisiaj(x.id),
      limit: (await polityka(x)).limitUsdNaDzien,
    })),
  )
        ...
        <div className="mx-auto max-w-strumien px-5 py-8">
          ...
          {braki.length > 0 && (
            <section className="mt-8">
              <h2 className="t-sekcja mb-1">Czego zabrakło w katalogu</h2>
              ...
              <ul className="divide-y overflow-hidden rounded-lg border bg-surface">
                {braki.map((w, i) => (
                  <li key={i} className="t-tresc flex gap-3 px-4 py-2.5">
          ...
          <section className="mt-8">
            <h2 className="t-sekcja mb-2">Dzisiejsze wydatki</h2>
            <ul className="divide-y overflow-hidden rounded-lg border bg-surface">
          ...
          <section className="mt-8">
            <h2 className="t-sekcja mb-2">Co się działo</h2>
            <ul className="divide-y overflow-hidden rounded-lg border bg-surface">
              {wpisy.length === 0 && <li className="t-meta px-4 py-3">Dziennik jest pusty.</li>}
              {wpisy.map((w, i) => {
                ...
                return (
                  <li key={i} className="flex gap-3 px-4 py-2.5">
                    ...
                    <span
                      className={`t-tresc min-w-0 flex-1 ${o.waga === "wazny" ? "" : "text-ink-2"}`}
                    >
