...
export async function POST(req: Request) {
  ...
  if (d.akcja === 'dodaj') {
    ...
    if (!/^https?:\/\//.test(d.url ?? '')) {
      return NextResponse.json({ blad: 'Adres musi zaczynać się od http:// albo https://.' }, { status: 400 })
    }
    if (!/^[a-z0-9-]{2,32}$/.test(d.nazwa ?? '')) {
      return NextResponse.json({ blad: 'Nazwa techniczna: małe litery, cyfry i myślnik.' }, { status: 400 })
    }
  ...
  if (d.akcja === 'przejrzyj') {
    ...
    try {
      ...
      await dziennik.zapisz(u.id, 'mcp.serwer.przejrzany', { serwer: s.nazwa, narzedzi: kandydaci.length })
      return NextResponse.json({
        kandydaci: kandydaci.map((k) => ({
          ...
          juzPrzyjete: s.narzedzia.some((n) => n.nazwaZdalna === k.nazwaZdalna && n.stan === 'zatwierdzone'),
      ...
      return NextResponse.json({ blad: `Nie udało się połączyć: ${String(e).slice(0, 160)}` }, { status: 502 })
    }
  }
  ...
  if (d.akcja === 'zatwierdz') {
    ...
    if (!d.opis?.trim() || !d.krotko?.trim()) {
      return NextResponse.json({ blad: 'Opis i krótka nazwa są wymagane — pisze je człowiek, nie serwer.' }, { status: 400 })
    }
    ...
    if (!k) return NextResponse.json({ blad: 'Serwer nie wystawia już tego narzędzia.' }, { status: 409 })
    if (k.odrzucone) return NextResponse.json({ blad: k.odrzucone }, { status: 422 })

    await katalog.zatwierdzNarzedzie(u.id, {
      serwer: s.nazwa, nazwaZdalna: d.nazwaZdalna,
      opis: d.opis.trim(), krotko: d.krotko.trim(),
      zdolnoscId: d.zdolnosc, odcisk: k.odciskDla(d.opis.trim()),
    })
    return NextResponse.json({ ok: true })
  }
