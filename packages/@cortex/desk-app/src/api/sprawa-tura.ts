...
import { dopiszZdarzenie, uruchomTure } from "@cortex/desk-core/runtime"
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (wydano >= p.limitUsdNaDzien) {
    return NextResponse.json(
      { blad: "Wyczerpany dzienny limit kosztów. Poproś przełożonego o podniesienie." },
      { status: 429 },
    )
  }
  const { tresc, zalaczniki } = await req.json()
  const pliki: string[] = Array.isArray(zalaczniki)
    ? zalaczniki.filter((z) => typeof z === "string").slice(0, 10)
    : []
  if (!tresc?.trim() && !pliki.length)
    return NextResponse.json({ blad: "Puste zlecenie." }, { status: 400 })
