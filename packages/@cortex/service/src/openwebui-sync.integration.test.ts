...
describe.skipIf(!hasDatabase)(
  "uzgodnienie rola -> grupa OpenWebUI — prawdziwy Postgres, HTTP zmockowane",
  () => {
    beforeEach(async () => {
      const db = getDb()
      await cleanup()
      ...
      const [role] = await db
        .insert(roles)
        .values({ code: ROLE_CODE, name: "Rola testowa OWUI" })
        .returning()
      roleId = role!.id
    ...
    async function createSecondRole(): Promise<string> {
      const [role] = await getDb()
        .insert(roles)
        .values({ code: SECOND_ROLE_CODE, name: "Druga rola OWUI" })
        .returning()
      return role!.id
    }
      ...
      it("BAZA odrzuca drugie mapowanie tej samej grupy — z pominięciem CAŁEJ warstwy serwisowej", async () => {
        ...
        const error = await getDb()
          ...
          .values({
            roleId: secondRoleId,
            groupId: GROUP_ID,
            groupName: `cortex:${SECOND_ROLE_CODE}`,
          })
    ...
    describe("reconcileRoleGroup — zbiór docelowy z żywej bazy", () => {
      ...
      it("użytkownik bez konta w OpenWebUI (nigdy się nie zalogował) jest pomijany BEZ błędu (D6)", async () => {
        const db = getDb()
        const [noAccountUser] = await db
          .insert(users)
          .values({ email: NO_ACCOUNT_EMAIL })
          .returning()
    ...
    describe("podpięcie/odpięcie (route openwebui-group, PUT)", () => {
      ...
      it("attachRoleGroup({kind:'existing'}) na nieistniejącej grupie zwraca group-not-found", async () => {
        ...
        const result = await attachRoleGroup({
          roleId,
          action: { kind: "existing", groupId: "brak-takiej" },
        })
