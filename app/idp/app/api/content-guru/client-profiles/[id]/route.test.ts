// Test funkcjonalny GET/PUT/DELETE /api/content-guru/client-profiles/:id —
// bramka jest osobno w ../../guard-coverage.test.ts. Tu: 404 (nigdy 403) dla
// "nie istnieje" i "cudzy" jednocześnie, scoping userEmail z gate.
...
const service = vi.hoisted(() => ({
  getMyClientProfile: vi.fn(async (userEmail: string, id: string) =>
    id === "foreign-or-missing" ? undefined : { id, userEmail, profileName: "Acme" },
  ),
  updateMyClientProfile: vi.fn(async (userEmail: string, id: string, input: { profileName: string }) =>
    id === "foreign-or-missing" ? undefined : { id, userEmail, ...input },
  ),
  deleteMyClientProfile: vi.fn(async (_userEmail: string, id: string) => id !== "foreign-or-missing"),
}))
...
function makeRequest(method: string, body?: unknown): Request {
  ...
  return new Request("http://localhost/api/content-guru/client-profiles/probe", {
    method,
    headers,
...
describe("GET /api/content-guru/client-profiles/:id", () => {
  it("200 dla własnego profilu", async () => {
    const response = await GET(makeRequest("GET") as never, ctx("1"))
    expect(response.status).toBe(200)
    expect(service.getMyClientProfile).toHaveBeenCalledWith(EMAIL, "1")
  })

  it("404 (nigdy 403) dla cudzego/nieistniejącego — nie zdradza który przypadek", async () => {
    const response = await GET(makeRequest("GET") as never, ctx("foreign-or-missing"))
    expect(response.status).toBe(404)
  })
})

describe("PUT /api/content-guru/client-profiles/:id", () => {
...
describe("DELETE /api/content-guru/client-profiles/:id", () => {
  it("200 happy path", async () => {
