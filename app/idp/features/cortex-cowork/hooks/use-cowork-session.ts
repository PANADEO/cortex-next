...
/**
 * Creates a sandbox session for the given project once (per app session) and
 * publishes its id to the shared cortex-cowork session store once ready.
 * Each project tile keeps its own live session.
 *
...
export function useEnsureCoworkSession(
  ...
  }, [sessionId, client, setSessionId, attempt, projectId])
