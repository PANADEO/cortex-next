...
export function MetricsBar({ totals }: { totals: UsageTotals }) {
  ...
  // Ten sam wybór użytkownika, który rozstrzyga napisy, rozstrzyga separator
  // tysięcy — inaczej karta ma etykietę po angielsku i liczbę po polsku.
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      <DataCard
        ...
        value={num(totals.totalTokens)}
        description={t("metrics.totalTokens.description", {
          request: num(totals.requestTokens),
          response: num(totals.responseTokens),
        })}
        icon={Layers}
      />
      <DataCard
        label={t("metrics.requestCount.label")}
        value={num(totals.requestCount)}
      ...
      <DataCard
        ...
        description={
          totals.userCount > totals.activeUsers
            ? t("metrics.activeUsers.withIdle", { total: num(totals.userCount) })
      ...
      <DataCard
        ...
        value={num(totals.averageTokensPerActiveUser)}
