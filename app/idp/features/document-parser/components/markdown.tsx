...
// Renderer wyniku ekstrakcji (job.markdown) — pełny strukturalny Markdown
// zwrócony przez model wizyjny (nagłówki, tabele, listy, obrazy jako
// [Image: opis], patrz prompt w services/document-parser/src/prompt.py).
// Kopia wzorca z app/idp/features/cortex-cowork/components/markdown.tsx
// (ten sam react-markdown+remark-gfm, już zależności repo) — lokalna, bo
// oba miejsca są specyficzne dla swojego modułu (code-service SKILL.md
// "Kiedy coś jest code-service, a kiedy nie": logika specyficzna dla jednego
// modułu zostaje lokalnie, nie każdy wspólny wzorzec wymaga wspólnego pakietu).
...
export function DocumentParserMarkdown({ content }: { content: string }) {
  return (
