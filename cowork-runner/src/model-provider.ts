...
const DEFAULT_MODEL = process.env.COWORK_MODEL ?? "anthropic/claude-sonnet-4-5"

/** Provider id used for OpenAI-compatible endpoints (cortex-proxy included). */
const OPENAI_COMPAT_PROVIDER_ID = "cortex-gateway"

export function readModelConfigFromEnv(): CoworkModelConfig | undefined {
  const raw = process.env.COWORK_MODEL_CONFIG
  if (!raw) return undefined
  try {
    return JSON.parse(raw) as CoworkModelConfig
  } catch {
    console.error("[cowork-runner] COWORK_MODEL_CONFIG is not valid JSON - ignoring")
    return undefined
  }
}
 ...
 */
export function configureModel(config: CoworkModelConfig | undefined): string {
